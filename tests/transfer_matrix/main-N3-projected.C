...
const OctahedralRep rr = OCTAHEDRAL_REP_A1g;
const OctahedralRep r = OCTAHEDRAL_REP_A1g;
...
int main()
{
  ...
  Vector3 Rv_ii, Rv_jj, Rv_kk, Rv_i, Rv_j, Rv_k;
          ...
          if (AA>=A) {
            ...
            *tmat_p = 0.0;
            ...
            for (int mm=0; mm<Rii.GetSubshellDimension(); ++mm) {
              ...
              Rv_jj = Rjj.GetVector(mm);
              Rv_kk = -1 * Mod( Rv_ii+Rv_jj ,dims, offset );

              for (int m=0; m<Ri.GetSubshellDimension(); ++m) {
                Rv_i = Ri.GetVector(m);
                Rv_j = Rj.GetVector(m);
                Rv_k  = -1 * Mod( Rv_i+Rv_j , dims, offset );

                *tmat_p += Rii.GetWeight(rr,mm) * Ri.GetWeight(r,m)  * T_uud_A( Rv_ii, Rv_jj, Rv_kk, Rv_i, Rv_j, Rv_k  );
