...
#define FOURTHORDER
  ...
  potential_arg.potential_type = POTENTIAL_TYPE_LIN;
  Potential virial_potential(potential_arg,1.0);

  //---- Construct source wavefunctions
  ...
  //---- Construct sink wavefunctions
  OneBody* sinks[N];
  for (int n=0; n<N; ++n) {
    sinks[n] = new OneBody(one_body_arg);
  }
  ...
  for (int n=0;n<N; ++n) {
    ...
    sprintf(f_name, "%s.%d", "results/pot", n); 
    file2[n].setf(ios_base::scientific);
    file2[n].open(f_name,ios_base::app);
  ...
  Propagator virial_propagator(propagator_arg);
...
//} 
    ...
    //---- Compute 1-<V> 
    for (int n=0; n<N; ++n) {
      virial_propagator.Set(propagator.Get(n), n); // Make a copy of the propagator at time t
      virial_potential.Evolve(virial_propagator.Get());
      sinks[n]->Set(virial_propagator.Get(n));
      corr = sinks[n]->Project(props[n]);
      sinks[n]->Set(props[n]);
      corr /= sinks[n]->Project(props[n]);
      corr -= 1.0;
      corr *= -1.0;
      file2[n] << setprecision(PREC) << corr.real() << " ";
      file2[n] << setprecision(PREC) << corr.imag() << " ";
    }
