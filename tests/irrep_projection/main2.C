...
const OctahedralRep r = OCTAHEDRAL_REP_Eg;
const Vector3 distinct_p(0,0,1);
const Vector3 distinct_q(0,1,1);
...
int main()
{
  ...
  OhProjector oh_projector(distinct_p, distinct_q, r);
  ...
  //for (int i=0; i<projector_dimension; ++i) {
  //  for (int j=0; j<projector_dimension; ++j) {
