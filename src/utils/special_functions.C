...
Float HermiteH(int n, Float x) // Returns h_n(x)
{
  ...
  for (int j=1; j<=n; ++j) {
    p3 = p2;
    p2 = p1;
    p1 = x*sqrt(2.0/j)*p2 - sqrt((Float)(j-1)/j)*p3;
  }
...
void HermiteH(Float* h, int n, Float x) { // Returns array h = { h_0(x), h_1(x), h_2(x), ...., h_{n-1}(x) }
  ...
  for (int j=1; j<n; ++j) {
    b = a;
    a = h[j-1];
    h[j] = x*sqrt(2.0/j)*a - sqrt((Float)(j-1)/j)*b;
  }
