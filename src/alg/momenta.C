...
Momenta::Momenta(const MomentaArg &momenta_arg)
{

  const char* fname = "Momenta::Momenta(const MomentaArg &)";
  VRB.Func(fname);
...
vector<int> Momenta::GetMomentum(int n)
{
  ...
  const char* fname = "vector<int> Momenta::GetMomentum(int)";
  VRB.Func(fname);
...
{
  const char* fname = "vector<int> Momenta::ShellCount(int)";
...
{
  const char* fname = "vector<int> Momenta::AccShellCount(int)";
  if (n < 0) { return 0; }
  if (n >= NumShells() ) { ERR.General(fname, "Invalid argument. Exceeds number of shells."); }
