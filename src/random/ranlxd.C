// ranlxd.C
// An implementation of Luscher's ranlxd code.
...
{
  const char* fname = "Ranlxd::Init(long)";
...
{
  const char* fname = "Ranlxd::LuxLevel(int)";
...
{
  const char* fname = "Ranlxd::GetState(long*)";
  int k;  

//  if (init==0)
//    ERR.General(fname,"Undefined state (generator is not initialized).");
...
{
  const char* fname = "Ranlxd::SetState(long*)";
  int k;  
...
RandomType Ranlxd::TypeQ() {
  const char* fname = "RandomType Ranlxd::TypeQ()";
  switch ( lux_level ) {
    case 1 : 
      return RANDOM_TYPE_RANLXD1; break;
    case 2 : 
      return RANDOM_TYPE_RANLXD2; break;
    default : 
      ERR.General(fname,"Unrecognized lux_level.");
  }
}
