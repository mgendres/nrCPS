...
Random::Random(const RandomArg &random_arg)
{
  const char* fname = "Random::Random(const RandomArg &)";
  VRB.Func(fname);
  ...
  switch(random_arg.seed_type) 
  {
    ...
    case SEED_TYPE_INPUT:
      ...
      seed = random_arg.seed;
      if (seed <= 0) ERR.General(fname, "Input seed must be positive.");
      break;  
    case SEED_TYPE_TIME:
...
Float Random::Gauss(Float sigma)
{
  Float x, y, r2;
  ...
  return sigma * y * sqrt (-2.0 * log (r2) / r2); // Box-Muller transform
}
...
Float Random::LogNormal(Float mu, Float sigma)
{
  return exp( Gauss(sigma) + mu );
}
...
{
  const char* fname = "Random::WriteState(const char*)";
  VRB.Func(fname);
...
{
  const char* fname = "Random::ReadState(const char*)";
  VRB.Func(fname);
