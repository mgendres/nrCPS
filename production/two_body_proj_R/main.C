...
//distinct_vecs[9] = Vector3(1,2,3);
  ...
  OneBody* one_bodies_A[n_reps];
  OneBody* one_bodies_B[n_reps];
  GeneralizedTwoBody* two_bodies[n_reps];
  for (int j=0; j<n_reps; ++j) {
    one_bodies_A[j] = new OneBody(one_body_arg);
    one_bodies_A[j]->Set(distinct_vecs[j]); 
    one_bodies_B[j] = new OneBody(one_body_arg);
    one_bodies_B[j]->Set(-1*distinct_vecs[j]); 
    
    two_body_arg.distinct_vec = distinct_vecs[j];
    two_bodies[j] = new GeneralizedTwoBody(two_body_arg);
  }
  ...
  Propagator propagator_A(prop_arg);
  Propagator propagator_B(prop_arg);
  ...
  for (int j=0; j<configurations; ++j) {
    ...
    for (int k=0; k<unload_period; ++k) {
      ...
      for (int n=0; n<N; ++n) {
        propagator_A.Set( one_bodies_A[n]->Get(), n);
        propagator_B.Set( one_bodies_B[n]->Get(), n);
      }
      ...
      for(int t=0; t<GJP.Tsites(); ++t) {
        ...
        for (int b=0; b<N; ++b) {
          ...
          corr[b][t] += two_bodies[b]->Run2( propagator_A.Get(b), propagator_B.Get(b), rep[b]);
        ...
        propagator_A.Run(hamiltonians);
        propagator_B.Run(hamiltonians);
  ...
  for (int b=0; b<N; ++b) {
    delete one_bodies_A[b];
    delete one_bodies_B[b];
    delete two_bodies[b];
    Fclose(file[b]);
  }
