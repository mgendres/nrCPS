...
void MultiLevel(int level, int branches, int steps, Propagator* prop1, Propagator* prop2,  TwoBody* two_body, Lattice* lattice, vector<Hamiltonian*> hamiltonians, complex<Float>* corr) {

  const char* fname = "void MultiLevel(int, int, int, Propagator*, Propagator*, TwoBody*, Lattice*, vector<Hamiltonian*>, complex<Float>*)";

  if (branches<1) { ERR.General(fname,"Number of branches is less than one."); }

  if (steps<1) { ERR.General(fname,"Number of steps per level is less than one."); }

  if (level>0) {

    // Save the current propagator state
    int vol = GJP.Vol();

    // Pointers to propagator sources
    Float* p1 = prop1->Get();
    Float* p2 = prop2->Get();

    // Allocate memory for saved sources and copy props to them
    Float source1[2*vol];
    Float source2[2*vol];
    for (int i=0; i<2*vol; ++i) { source1[i] = p1[i]; }
    for (int i=0; i<2*vol; ++i) { source2[i] = p2[i]; }

    int levels = GJP.Tsites()/steps;
    Float norm = pow( (Float)branches, (Float)(levels - level + 1));

    // For each branch within the level
    for (int i=0; i< branches; ++i) {

      // Set the passed propagator to that of the saved state
      prop1->SetSource(source1);
      prop2->SetSource(source2);

      // Do an independent evolution of the propagators
      for (int s=0; s<steps; ++s) {
  
        //---- Perform contractions
        int t = GJP.Tsites()-(level-1)*steps-(steps-s);
//        printf("level = %d, branch = %d, step = %d, time = %d\n", level, i, s, t);
        corr[t] += (two_body->Run(p1, p2) / norm);
      ...
      // Then branch again
      MultiLevel(level-1, branches, steps,  prop1, prop2, two_body, lattice, hamiltonians, corr);
...
//      }
      ...
      int branches = 2;
      int steps = 4;
      if (GJP.Tsites() != levels*steps) { ERR.General(fname,"Houston, we've had a problem."); }

      MultiLevel(levels, branches, steps, &prop1, &prop2, &two_body, &lattice2, hamiltonians, corr);
