...
#define IMPROVED 
  ...
  //---- Number of cumulants ranging from [0,inf)
  int Nc = 6;
  if (Nc < 0) {  ERR.General(fname,"Number of cumulants (Nc) must be greater than or equal to zero."); }
  FILE* zfile[Nc][2*N];
  ...
  for (int n=0; n<2*N; ++n) {
    ...
    for (int nc=0; nc<Nc; ++nc) {
      //---- This is for the single particle propagators
      sprintf(f_name, "%s%d.%d", "results/z", nc+1, n+1); 
      zfile[nc][n] = Fopen(IO_TYPE_ROOT,f_name,"a");
    }
  ...
  complex<Float> z[Nc][2*N][GJP.Tsites()];
  for (int n=0; n<2*N; ++n)
  ...
  for (int j=0; j<configurations; ++j) {
    ...
    for (int k=0; k<unload_period; ++k) {
      ...
      for(int t=0; t<GJP.Tsites(); ++t) {
        ...
        for (int n=0; n<N; ++n) {
          ...
          slat = slater_det.Run(n+1, -1, -1);
          corr[2*n+1][t] += slat;
          if ( Nc>0 ) {
            cum[0] = log(abs(slat)); // Is this correct?
            for (int nc=1; nc<Nc; ++nc) { cum[nc] = cum[0]*cum[nc-1]; }
            for (int nc=0; nc<Nc; ++nc) { z[nc][2*n+1][t] += cum[nc]; }
          }
          
          //---- Case 2: n_up = n+1, n_down = n
          slat = slater_det.Run(n+1, -1, n); 
          corr[2*n][t] += slat;
          if ( Nc>0 ) {
            cum[0] = log(abs(slat)); // Is this correct?
            for (int nc=1; nc<Nc; ++nc) { cum[nc] = cum[0]*cum[nc-1]; }
            for (int nc=0; nc<Nc; ++nc) { z[nc][2*n][t] += cum[nc]; }
          }
        }
    ...
    Comms::GlobalSum( &(z[0][0][0]), Nc * 2*N * GJP.Tsites() );
    ...
    for (int n=0; n<2*N; ++n) {
      for(int t=0; t<GJP.Tsites(); ++t) {
        ...
        //---- Store accumulated results in file, then reset accumulated corr
        Fprintf(file[n], "%.*e %.*e ", PREC, corr[n][t].real() / block_size, PREC, corr[n][t].imag() / block_size);
        corr[n][t]=0.0;

        for (int nc=0; nc<Nc; ++nc) {
          //---- Store accumulated results in file, then reset accumulated cumulant
          //Fprintf(zfile[nc][n], "%.*e %.*e ", PREC, z[nc][n][t].real() / block_size, PREC, z[nc][n][t].imag() / block_size);
          Fprintf(zfile[nc][n], "%.*e ", PREC, z[nc][n][t].real() / block_size);
          z[nc][n][t]=0.0;
        }
